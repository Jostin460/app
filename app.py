...
# ----------------------------
st.title("🧠 Mental Health Data Analysis Assistant")
...
question = st.text_area("✍️ Escribe tu pregunta sobre el dataset:",
                        placeholder="Ejemplo: ¿Cuál es el país con mayor porcentaje de depresión en 2015?")
...
# CARGA DEL DATASET
# ----------------------------
uploaded_file = st.file_uploader("📂 Sube tu archivo CSV (dataset de salud mental)", type="csv")
...
if st.button("🔍 Analizar pregunta"):
    ...
    system_prompt = (
        ...
        "'Entity' (país o región), 'Code' (código del país), 'Year' (año del registro), "
        "'Schizophrenia (%)', 'Bipolar disorder (%)', 'Eating disorders (%)', "
        "'Anxiety disorders (%)', 'Drug use disorders (%)', 'Depression (%)', "
        "y 'Alcohol use disorders (%)'. "
        "El dataset muestra la prevalencia porcentual de diferentes trastornos mentales en distintos países y años. "
        "Si la pregunta del usuario NO está relacionada con este dataset o con análisis de salud mental, "
        "responde amablemente diciendo que no puedes responder porque la pregunta está fuera del alcance del conjunto de datos."
    )
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Aquí están los primeros datos:\n{df_string}"},
        ...
        st.error(f"⚠️ Error al consultar el modelo: {e}")